...
public interface CashDAO {
    ...
    @Query("SELECT IFNULL(" +
            ...
            "FROM Cash E1 " +
            "WHERE E1.type = 'E' AND E1.payment = 'DIN' AND E1.identifier >= (SELECT MAX(E2.identifier) FROM Cash E2 WHERE E2.operation == 'ABE')) " +
            "-" +
            "(SELECT IFNULL(SUM(S1.total), 0) " +
            "FROM Cash S1 " +
            "WHERE S1.type = 'S' AND S1.payment = 'DIN' AND S1.identifier >= (SELECT MAX(S2.identifier) FROM Cash S2 WHERE S2.operation == 'ABE')) " +
            ", 0)")
    ...
    @Query("SELECT IFNULL(" +
            "(SELECT IFNULL(SUM(E1.total), 0) " +
            "FROM Cash E1 " +
            "WHERE E1.type = 'E' AND E1.payment = 'CAR' AND E1.identifier >= (SELECT MAX(E2.identifier) FROM Cash E2 WHERE E2.operation == 'ABE')) " +
            "-" +
            "(SELECT IFNULL(SUM(S1.total), 0) " +
            "FROM Cash S1 " +
            "WHERE S1.type = 'S' AND S1.payment = 'CAR' AND S1.identifier >= (SELECT MAX(S2.identifier) FROM Cash S2 WHERE S2.operation == 'ABE')) " +
            ", 0)")
    ...
    @Query("SELECT C1.operation AS 'operation', C1.payment AS 'payment', C1.dateTime AS 'dateTime', C1.total AS 'total', C1.type AS 'type' " +
            "FROM Cash C1 " +
            "WHERE C1.identifier = (SELECT MAX(S1.identifier) FROM Cash S1 WHERE S1.operation = 'ABE') AND C1.operation = 'ABE' " +
            "UNION ALL " +
            "SELECT C2.operation AS 'operation', C2.payment AS 'payment', MAX(C2.dateTime) AS 'dateTime', SUM(C2.total) AS 'total', C2.type AS 'type' " +
            "FROM Cash C2 " +
    ...
    @Query(" SELECT C1.operation AS 'operation', C1.payment AS 'payment', C1.dateTime AS 'dateTime', C1.total AS 'total', C1.type AS 'type', U1.name AS 'user_name' " +
            "FROM Cash C1 " +
            "INNER JOIN User U1 on U1.identifier = C1.user " +
            "WHERE C1.identifier = (SELECT MAX(S1.identifier) FROM Cash S1 WHERE S1.operation = 'ABE') AND C1.operation = 'ABE' " +
            "UNION ALL " +
            "SELECT C2.operation AS 'operation', C2.payment AS 'payment', MAX(C2.dateTime) AS 'dateTime', SUM(C2.total) AS 'total', C2.type AS 'type', U2.name AS 'user_name' " +
            "FROM Cash C2 " +
            ...
            "GROUP BY 1, 2, 5 " +
            "UNION ALL " +
            "SELECT C3.operation AS 'operation', C3.payment AS 'payment', C3.dateTime AS 'dateTime', C3.total AS 'total', C3.type AS 'type', U3.name AS 'user_name' " +
            "FROM Cash C3 " +
            ...
            "UNION ALL " +
            "SELECT C4.operation AS 'operation', C4.payment AS 'payment', C4.dateTime AS 'dateTime', C4.total AS 'total', C4.type AS 'type', U4.name AS 'user_name' " +
            "FROM Cash C4 " +
            ...
            "UNION ALL " +
            "SELECT C5.operation AS 'operation', C5.payment AS 'payment', C5.dateTime AS 'dateTime', C5.total AS 'total', C5.type AS 'type', U5.name AS 'user_name' " +
            "FROM Cash C5 " +
